...
export function sha256Digest(value: unknown): string { return `sha256:${sha256Hex(value)}`; }
export function sha256TextDigest(value: string): string { return `sha256:${createHash("sha256").update(value, "utf8").digest("hex")}`; }

export function toJsonObject(value: unknown): JsonObject {
  const normalized = normalizeJson(value);
  if (normalized === null || Array.isArray(normalized) || typeof normalized !== "object") throw new TypeError("Expected a JSON object");
  return normalized;
}
