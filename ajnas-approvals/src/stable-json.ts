...
function normalize(value: unknown, state: NormalizeState, depth: number): JsonValue {
  if (depth > MAX_DEPTH) {
    throw new RangeError("JSON value exceeds the maximum depth of 64");
  }
  state.nodes += 1;
  if (state.nodes > MAX_NODES) {
    throw new RangeError("JSON value exceeds the 100000-node limit");
  }
  ...
  if (typeof value === "string") {
    accountBytes(state, value);
    return value;
  }
  if (typeof value === "number") {
    if (!Number.isFinite(value) || Object.is(value, -0)) {
      throw new TypeError("JSON numbers must be finite and must not be negative zero");
    }
    return value;
  }
  ...
  if (state.seen.has(value)) {
    throw new TypeError("Cannot serialize cyclic or repeated object references");
  }
...
function assertDenseDataArray(value: unknown[]): void {
  ...
  if (keys.length !== value.length + 1 || !keys.includes("length")) {
    throw new TypeError("JSON arrays must be dense and must not contain extra properties");
  }
...
function accountBytes(state: NormalizeState, value: string): void {
  ...
  if (state.bytes > MAX_UTF8_BYTES) {
    throw new RangeError("JSON string and key data exceeds the 8 MiB limit");
  }
}
