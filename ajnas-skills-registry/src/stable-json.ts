...
export function sanitizeJson(value: unknown): JsonValue { return normalizeJson(value); }

function normalizeJson(value: unknown): JsonValue { return normalize(value, { seen: new WeakSet(), nodes: 0, bytes: 0 }, 0); }
function normalize(value: unknown, state: NormalizeState, depth: number): JsonValue {
  ...
  for (const key of Reflect.ownKeys(value).sort(comparePropertyKeys)) {
    ...
    Object.defineProperty(output, key, { value: normalize(descriptor.value, state, depth + 1), enumerable: true, configurable: true, writable: true });
  }
  return output;
}
