...
function normalize(value: unknown, state: NormalizeState, depth: number): JsonValue {
  ...
  if (Array.isArray(value)) { assertDenseDataArray(value); return value.map((item) => normalize(item, state, depth + 1)); }
