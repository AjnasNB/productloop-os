...
export function sha256Digest(value: unknown): string { return `sha256:${createHash("sha256").update(stableJson(value)).digest("hex")}`; }
export function sha256Hex(value: string): string { return createHash("sha256").update(value).digest("hex"); }
export function toJsonValue(value: unknown): JsonValue { return normalizeJson(value); }
export function toJsonObject(value: unknown): JsonObject {
  const normalized = normalizeJson(value);
  return normalized !== null && typeof normalized === "object" && !Array.isArray(normalized) ? normalized : createObject([["value", normalized]]);
}
export function normalizeJson(value: unknown): JsonValue { return normalize(value, { seen: new WeakSet(), nodes: 0, bytes: 0 }, 0, "$"); }

function normalize(value: unknown, state: NormalizeState, depth: number, path: string): JsonValue {
  ...
  if (typeof value !== "object") throw new TypeError(`Cannot serialize non-JSON value of type ${typeof value} at ${path}`);
  if (state.seen.has(value)) throw new TypeError(`Cannot serialize cyclic or repeated object references at ${path}`);
  state.seen.add(value);
  if (Array.isArray(value)) { assertDenseDataArray(value); return value.map((item, index) => normalize(item, state, depth + 1, `${path}[${index}]`)); }
